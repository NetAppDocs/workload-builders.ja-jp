...
summary: Workload Factory for Databases で Microsoft SQL Server インスタンスを管理します。 
---
= ビルダープロジェクトの管理
:allow-uri-read: 
:icons: font
...
[role="lead"]
NetApp Workload Factory for Builders で Builders プロジェクトを管理して、各プロジェクトのコードとアーティファクトの管理方法を制御できます。
...
次の手順に従って、 NetApp Workload Factory for Builders で作成された既存のプロジェクトを表示できます。
